...
\date{Package version \texttt{\Sexpr{sversion}}\\ Last edited December 2013}
\maketitle
...
Note that times are stored in {\bf minutes}.
...
Using other options to the \texttt{haldane} command, it is possible to
calculate quantities such as the depth ceiling (the shallowest permissible depth
given the diver's tissue saturation), the pressure ceiling (the lowest
permissible ambient pressure given the diver's tissue saturation)
and the washout (difference between
...
\noindent 
To find the equivalent narcotic depth a dive to 40 metres on Trimix 18/50
...
then 
\begin{quote}
  {\bf the package will not assume you breathe air at the surface.}
\end{quote}
...
Note that {\bf elapsed times are always calculated in minutes} so the
...
(note again that the times are in {\bf minutes})
...
\verb!<Adrian.Baddeley@uwa.edu.au>!
